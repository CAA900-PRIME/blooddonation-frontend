...
const Signup = () => {
	...
	const handleCreateAccount = async () => {
		const { username, firstName, lastName, dob, email, password, confirmPassword, phoneNumber, homeAddress, city, country, postalCode } = formData;

		if (!firstName || !lastName || !dob || !email || !password || !confirmPassword || !phoneNumber || !homeAddress || !city || !country || !postalCode, !username) {
		...
		try {
			const response = await fetch("http://localhost:3000/api/auth/signup", {
				...
				body: JSON.stringify({ firstName, lastName, dob, email, password, phoneNumber, homeAddress, city, country, postalCode, username }),
			});
